'use client'
import { useState, useEffect } from 'react'
import { createClient } from '@/lib/supabase/client'
import { calculateAutoScore, COUNTRY_LABELS, getImportCost } from '@/lib/utils'
import { Dialog, DialogContent, DialogHeader, DialogTitle } from '@/components/ui/dialog'
import { Button } from '@/components/ui/button'
import { Input } from '@/components/ui/input'
import { Label } from '@/components/ui/label'
import { Textarea } from '@/components/ui/textarea'
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/components/ui/select'
import { Checkbox } from '@/components/ui/checkbox'
import type { Client, Listing } from '@/types/database'

interface ListingFormModalProps {
  open: boolean
  onClose: () => void
  onSaved: () => void
  listing?: Listing | null
  defaultClientId?: string
}

const FUELS = ['Essence', 'Diesel', 'Hybride', 'Électrique', 'GPL']
const GEARBOXES = ['Manuelle', 'Automatique']
const BODIES = ['Berline', 'SUV/4x4', 'Citadine', 'Break', 'Coupé', 'Cabriolet', 'Utilitaire', 'Monospace']
const SOURCES = ['AutoScout24', 'La Centrale', 'LeBonCoin', 'mobile.de', 'Le Parking', 'Manuel', 'Chat']
const STATUSES = [
  { value: 'new', label: 'Nouveau' },
  { value: 'viewed', label: 'Vu' },
  { value: 'contacted', label: 'Contacté' },
  { value: 'negotiation', label: 'Négociation' },
  { value: 'bought', label: 'Acheté' },
  { value: 'resold', label: 'Revendu' },
  { value: 'ignored', label: 'Ignoré' },
]

export function ListingFormModal({ open, onClose, onSaved, listing, defaultClientId }: ListingFormModalProps) {
  const [clients, setClients] = useState<Client[]>([])
  const [loading, setLoading] = useState(false)
  const [tagInput, setTagInput] = useState('')

  const [form, setForm] = useState({
    brand: listing?.brand ?? '',
    model: listing?.model ?? '',
    generation: listing?.generation ?? '',
    year: listing?.year?.toString() ?? '',
    km: listing?.km?.toString() ?? '',
    price: listing?.price?.toString() ?? '',
    fuel: listing?.fuel ?? '',
    gearbox: listing?.gearbox ?? '',
    body: listing?.body ?? '',
    country: listing?.country ?? 'FR',
    seller: listing?.seller ?? '',
    first_owner: listing?.first_owner ?? false,
    url: listing?.url ?? '',
    source: listing?.source ?? '',
    notes: listing?.notes ?? '',
    status: listing?.status ?? 'new',
    tags: listing?.tags ?? [] as string[],
    client_id: listing?.client_id ?? defaultClientId ?? '',
    manual_score: listing?.manual_score?.toString() ?? '',
  })

  useEffect(() => {
    async function fetchClients() {
      const supabase = createClient()
      const { data: { user } } = await supabase.auth.getUser()
      if (!user) return
      const { data } = await supabase.from('clients').select('id, name').eq('user_id', user.id).order('name')
      setClients((data as Client[]) ?? [])
    }
    fetchClients()
  }, [])

  function updateField(key: string, value: string | boolean | string[]) {
    setForm(prev => ({ ...prev, [key]: value }))
  }

  function addTag() {
    const tag = tagInput.trim()
    if (tag && !form.tags.includes(tag)) {
      updateField('tags', [...form.tags, tag])
    }
    setTagInput('')
  }

  function removeTag(tag: string) {
    updateField('tags', form.tags.filter(t => t !== tag))
  }

  const autoScore = calculateAutoScore({
    year: form.year ? parseInt(form.year) : null,
    km: form.km ? parseInt(form.km) : null,
    price: form.price ? parseInt(form.price) : null,
    seller: form.seller,
    first_owner: form.first_owner,
  })

  const importCost = getImportCost(form.country)

  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    if (!form.brand) return
    setLoading(true)

    try {
      const supabase = createClient()
      const { data: { user } } = await supabase.auth.getUser()
      if (!user) return

      const payload = {
        user_id: user.id,
        brand: form.brand,
        model: form.model || null,
        generation: form.generation || null,
        year: form.year ? parseInt(form.year) : null,
        km: form.km ? parseInt(form.km) : null,
        price: form.price ? parseInt(form.price) : null,
        fuel: form.fuel || null,
        gearbox: form.gearbox || null,
        body: form.body || null,
        country: form.country || null,
        seller: form.seller || null,
        first_owner: form.first_owner,
        url: form.url || null,
        source: form.source || null,
        notes: form.notes || null,
        status: form.status,
        tags: form.tags.length > 0 ? form.tags : null,
        auto_score: autoScore,
        manual_score: form.manual_score ? parseInt(form.manual_score) : null,
        client_id: form.client_id || null,
        updated_at: new Date().toISOString(),
      }

      if (listing) {
        await supabase.from('listings').update(payload).eq('id', listing.id)
      } else {
        await supabase.from('listings').insert(payload)
      }

      onSaved()
    } finally {
      setLoading(false)
    }
  }

  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>{listing ? 'Modifier l\'annonce' : 'Nouvelle annonce'}</DialogTitle>
        </DialogHeader>

        <form onSubmit={handleSubmit} className="space-y-4">
          {/* Marque / Modèle / Génération */}
          <div className="grid grid-cols-3 gap-3">
            <div className="space-y-1.5">
              <Label>Marque *</Label>
              <Input placeholder="BMW, Audi…" value={form.brand} onChange={e => updateField('brand', e.target.value)} required />
            </div>
            <div className="space-y-1.5">
              <Label>Modèle</Label>
              <Input placeholder="320d, A4…" value={form.model} onChange={e => updateField('model', e.target.value)} />
            </div>
            <div className="space-y-1.5">
              <Label>Génération</Label>
              <Input placeholder="G20, B9…" value={form.generation} onChange={e => updateField('generation', e.target.value)} />
            </div>
          </div>

          {/* Année / KM / Prix */}
          <div className="grid grid-cols-3 gap-3">
            <div className="space-y-1.5">
              <Label>Année</Label>
              <Input type="number" placeholder="2020" value={form.year} onChange={e => updateField('year', e.target.value)} min="1990" max="2030" />
            </div>
            <div className="space-y-1.5">
              <Label>Kilométrage</Label>
              <Input type="number" placeholder="80000" value={form.km} onChange={e => updateField('km', e.target.value)} />
            </div>
            <div className="space-y-1.5">
              <Label>Prix (€)</Label>
              <Input type="number" placeholder="15000" value={form.price} onChange={e => updateField('price', e.target.value)} />
            </div>
          </div>

          {/* Carburant / Boîte / Carrosserie */}
          <div className="grid grid-cols-3 gap-3">
            <div className="space-y-1.5">
              <Label>Carburant</Label>
              <Select value={form.fuel} onValueChange={v => updateField('fuel', v)}>
                <SelectTrigger><SelectValue placeholder="Carburant" /></SelectTrigger>
                <SelectContent>{FUELS.map(f => <SelectItem key={f} value={f}>{f}</SelectItem>)}</SelectContent>
              </Select>
            </div>
            <div className="space-y-1.5">
              <Label>Boîte</Label>
              <Select value={form.gearbox} onValueChange={v => updateField('gearbox', v)}>
                <SelectTrigger><SelectValue placeholder="Boîte" /></SelectTrigger>
                <SelectContent>{GEARBOXES.map(g => <SelectItem key={g} value={g}>{g}</SelectItem>)}</SelectContent>
              </Select>
            </div>
            <div className="space-y-1.5">
              <Label>Carrosserie</Label>
              <Select value={form.body} onValueChange={v => updateField('body', v)}>
                <SelectTrigger><SelectValue placeholder="Type" /></SelectTrigger>
                <SelectContent>{BODIES.map(b => <SelectItem key={b} value={b}>{b}</SelectItem>)}</SelectContent>
              </Select>
            </div>
          </div>

          {/* Pays / Vendeur / Statut */}
          <div className="grid grid-cols-3 gap-3">
            <div className="space-y-1.5">
              <Label>Pays</Label>
              <Select value={form.country} onValueChange={v => updateField('country', v)}>
                <SelectTrigger><SelectValue placeholder="Pays" /></SelectTrigger>
                <SelectContent>
                  {Object.entries(COUNTRY_LABELS).map(([code, label]) => (
                    <SelectItem key={code} value={code}>{label}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
            <div className="space-y-1.5">
              <Label>Vendeur</Label>
              <Select value={form.seller} onValueChange={v => updateField('seller', v)}>
                <SelectTrigger><SelectValue placeholder="Type vendeur" /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="particulier">Particulier</SelectItem>
                  <SelectItem value="professionnel">Professionnel</SelectItem>
                </SelectContent>
              </Select>
            </div>
            <div className="space-y-1.5">
              <Label>Statut</Label>
              <Select value={form.status} onValueChange={v => updateField('status', v)}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>{STATUSES.map(s => <SelectItem key={s.value} value={s.value}>{s.label}</SelectItem>)}</SelectContent>
              </Select>
            </div>
          </div>

          {/* Source / Client */}
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              <Label>Source</Label>
              <Select value={form.source} onValueChange={v => updateField('source', v)}>
                <SelectTrigger><SelectValue placeholder="Où avez-vous trouvé?" /></SelectTrigger>
                <SelectContent>{SOURCES.map(s => <SelectItem key={s} value={s}>{s}</SelectItem>)}</SelectContent>
              </Select>
            </div>
            <div className="space-y-1.5">
              <Label>Client associé</Label>
              <Select value={form.client_id || 'none'} onValueChange={v => updateField('client_id', v === 'none' ? '' : v)}>
                <SelectTrigger><SelectValue placeholder="Aucun client" /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="none">Aucun client</SelectItem>
                  {clients.map(c => <SelectItem key={c.id} value={c.id}>{c.name}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
          </div>

          {/* URL */}
          <div className="space-y-1.5">
            <Label>Lien annonce</Label>
            <Input placeholder="https://…" value={form.url} onChange={e => updateField('url', e.target.value)} type="url" />
          </div>

          {/* Options */}
          <div className="flex items-center gap-6">
            <label className="flex items-center gap-2 cursor-pointer">
              <Checkbox checked={form.first_owner} onCheckedChange={v => updateField('first_owner', v as boolean)} />
              <span className="text-sm text-gray-300">1ère main</span>
            </label>
          </div>

          {/* Scores */}
          <div className="grid grid-cols-2 gap-3">
            <div className="p-3 rounded-lg bg-[#0d1117] border border-[#2a2f3e]">
              <p className="text-xs text-gray-500 mb-1">Score auto calculé</p>
              <p className="text-2xl font-bold text-orange-400">{autoScore}/100</p>
            </div>
            <div className="space-y-1.5">
              <Label>Score manuel (0-100)</Label>
              <Input type="number" placeholder="Optionnel" value={form.manual_score} onChange={e => updateField('manual_score', e.target.value)} min="0" max="100" />
            </div>
          </div>

          {/* Import cost */}
          {importCost > 0 && (
            <div className="p-3 rounded-lg bg-yellow-900/20 border border-yellow-800/50">
              <p className="text-xs text-yellow-400">
                ⚠️ Coût d'import estimé depuis {COUNTRY_LABELS[form.country]} : <strong>+{importCost}€</strong>
              </p>
            </div>
          )}

          {/* Tags */}
          <div className="space-y-1.5">
            <Label>Tags</Label>
            <div className="flex gap-2">
              <Input
                placeholder="Ajouter un tag…"
                value={tagInput}
                onChange={e => setTagInput(e.target.value)}
                onKeyDown={e => e.key === 'Enter' && (e.preventDefault(), addTag())}
              />
              <Button type="button" variant="secondary" onClick={addTag} size="sm">Ajouter</Button>
            </div>
            {form.tags.length > 0 && (
              <div className="flex flex-wrap gap-1.5 mt-2">
                {form.tags.map(tag => (
                  <span key={tag} className="inline-flex items-center gap-1 px-2 py-0.5 text-xs rounded-full bg-orange-900/30 text-orange-300 border border-orange-700/50">
                    {tag}
                    <button type="button" onClick={() => removeTag(tag)} className="ml-1 hover:text-white">×</button>
                  </span>
                ))}
              </div>
            )}
          </div>

          {/* Notes */}
          <div className="space-y-1.5">
            <Label>Notes</Label>
            <Textarea placeholder="Remarques, observations…" value={form.notes} onChange={e => updateField('notes', e.target.value)} />
          </div>

          <div className="flex justify-end gap-2 pt-2">
            <Button type="button" variant="secondary" onClick={onClose}>Annuler</Button>
            <Button type="submit" disabled={loading || !form.brand}>
              {loading ? 'Enregistrement…' : listing ? 'Mettre à jour' : 'Ajouter'}
            </Button>
          </div>
        </form>
      </DialogContent>
    </Dialog>
  )
}
